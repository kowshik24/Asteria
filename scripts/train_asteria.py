...
def main():
    args = parse_args()
    device = args.device
    emb = np.load(args.train_embeddings)  # shape (N, dim)
    ...
    for epoch in range(args.epochs):
        ...
        for step in range(args.steps_per_epoch):
            ...
            # Pair mining for positive/negative structure (original space)
            ...
            # Quantization smoothness: encourage small variance inside blocks after scaling
            q_codes = lrsq.quantize(proj.detach())  # detach to not explode grad
            ...
            # Re-orthonormalize LRSQ projection every few steps
            if step % 50 == 0:
